    ...
    >
      ...
      <main style={{ maxWidth: 1040, margin: "0 auto", padding: "24px 20px 50px" }}>
        <button
          ...
          aria-label="Go back"
          style={{
            border: "none", background: "transparent", marginBottom: 12,
            padding: 0, cursor: "pointer", color: "#777", fontSize: 16,
        ...
        <div style={{ marginBottom: 18 }}>
          <h1 style={{ margin: 0, fontSize: 26, fontWeight: 800, color: "#2d2d2d" }}>
            Your Favorites
          </h1>
        ...
        {loading ? (
          <div style={{ color: "#999", padding: "30px 0", fontSize: 14 }} role="status">
            Loading…
          </div>
        ) : items.length === 0 ? (
          <div
            style={{
              ...
              padding: "36px 24px", textAlign: "center", color: "#777",
              width: 560, maxWidth: "100%", boxShadow: "0 4px 12px rgba(0,0,0,0.04)",
            }}
          >
            <div style={{ fontSize: 42, marginBottom: 10 }}>♡</div>
            <p style={{ fontSize: 17, fontWeight: 700, color: "#555", margin: "0 0 8px" }}>
              No favorites yet
            </p>
            <p style={{ fontSize: 14, color: "#999", margin: "0 0 18px", lineHeight: 1.5 }}>
              Save restaurants you love so you can find them easily later.
            </p>
            <button
              onClick={() => navigate("/search?q=Restaurants&loc=")}
              style={{
                background: "#d32323", color: "#fff", border: "none", borderRadius: 8,
          ...
          <section aria-label="Your favourite restaurants">
            ...
            <div style={{ display: "grid", gap: 14, width: 560, maxWidth: "100%" }}>
              {items.map((item) => (
                <article key={item.id} style={cardBase} aria-label={item.name}>
                  <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", gap: 12 }}>
                    <div
                      role="button"
                      tabIndex={0}
                      onClick={() => navigate(`/restaurant/${item.id}`)}
                      onKeyDown={(e) => e.key === "Enter" && navigate(`/restaurant/${item.id}`)}
                      style={{ cursor: "pointer", display: "flex", gap: 12, alignItems: "flex-start", flex: 1, minWidth: 0 }}
                    >
                      <div style={imgBox}>
                        {item.photos?.[0] || item.image ? (
                          <img src={item.photos?.[0] || item.image} alt={item.name} style={imgStyle} />
                        ) : "🍽️"}
                      </div>

                      <div style={{ flex: 1, minWidth: 0 }}>
                        <div style={{ fontWeight: 800, fontSize: 16, color: "#2d2d2d", lineHeight: 1.2, marginBottom: 6 }}>
                          {item.name}
                        </div>
                        ...
                        <div style={{ display: "flex", alignItems: "center", gap: 7, flexWrap: "wrap", fontSize: 13 }}>
                          ...
                          <span style={{ color: "#999" }}>
                            ({item.review_count || 0} {(item.review_count || 0) === 1 ? "review" : "reviews"})
                          </span>
                          {typeof item.is_open === "boolean" && (
                            <span style={{ color: item.is_open ? "#166534" : "#d32323", fontWeight: 700, fontSize: 12 }}>
                    ...
                    <button
                      ...
                      style={favBtn}
                      onClick={(e) => { e.stopPropagation(); handleUnfavorite(item.id); }}
                    >
                      ★
                    </button>
                  </div>
...
const favBtn = {
  border: "none", background: "transparent", color: "#d32323",
